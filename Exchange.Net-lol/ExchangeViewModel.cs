...
namespace Exchange.Net
{
    public abstract class ExchangeViewModel : ReactiveObject
    {
            ...
            else
            {
                ...
                foreach (var market in markets)
                {
                    ...
                    if (symbols.ContainsKey(id))
                    {
                        ...
                        {
                            if (oldMarket.Status != market.Status)
                            {
                                oldMarket.Status = market.Status;
                            }
                            if (!IsValidMarket(oldMarket))
                                RemoveMarketSummary(id);
                        }
                    }
                    else
                    {
                        ...
                        symbols.TryAdd(market.Symbol, market);
                        Markets.Add(market);
                        // TODO: verify below is needed or not.
                        if (IsValidMarket(market))
                            AddMarketSummary(market);
                    }
                }
                // remove orphaned.
                var oldMarkets = symbols.Keys.Except(markets.Select(m => m.Symbol));
                foreach (var market in oldMarkets)
                {
                    if (symbols.TryRemove(market, out SymbolInformation oldMarket))
                    {
                        Markets.Remove(oldMarket);
                        RemoveMarketSummary(market);
                    }
                }
            }
        }
        ...
        {
            var ticker = MarketSummaries.SingleOrDefault(t => t.Symbol == id);
            if (ticker != null)
            {
                Debug.Assert(MarketSummaries.Remove(ticker));
                Debug.Assert(marketSummariesHash.Remove(ticker.Symbol));
            }
        }
        ...
        protected SerialDisposable PublicTradesHandle { get; } = new SerialDisposable();
