    ...
    public class BinanceViewModel : ExchangeViewModel
    {
		...
		public override IObservable<PriceTicker> SubscribeMarketSummaries(IEnumerable<string> symbols, string period)
        {
            ...
            else
            {
                ...
                var obs = client.ObserveKlineCandlesticks(symbols, period);
                UpdateStatus();
                return obs2.Select(ToPriceTicker).Concat(obs.Select(ToPriceTicker));
            }
        }
        ...
        internal static PublicTrade ToPublicTrade(Binance.WsTrade x)
        {
            return new PublicTrade()
            {
                Id = x.tradeId,
        ...
        private async void Initialize()
        {
            ...
            if (crExchangeInfo.Success)
                ProcessExchangeInfo(crExchangeInfo.Data.symbols.Select(CreateSymbolInformation));
        ...
        protected IObservable<PublicTrade> SubscribeRecentTrades(string symbol, int limit)
        {
            var task = Task.Run(() => client.GetRecentTrades2(symbol, limit));
            var obs2 = task.ToObservable().SelectMany(x => x.Data.ToObservable());
            var obs = client.SubscribePublicTradesAsync(symbol, TradesMaxItemCount);
            UpdateStatus();
            return obs2.Select(ToPublicTrade).Do((t) => t.Symbol = symbol).Finally(() => Debug.Print("Recent trades done")).Concat(obs.Select(ToPublicTrade));
        }
