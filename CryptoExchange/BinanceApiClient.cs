...
namespace Exchange.Net
{
    public class BinanceApiClient : ExchangeApiCore
    {
        ...
        public IObservable<Binance.ServerTime> SubscribeServerTime()
        {
            return Observable.Interval(TimeSpan.FromSeconds(2)).Select(
               (long x) =>
               {
                   var result = GetServerTime2();
                   return result;
               }).Where(result => result.Success).Select(result => result.Data);
        }
        ...
        public IObservable<Binance.ExchangeInfo> SubscribeExchangeInfo()
        {
            return Observable.Interval(TimeSpan.FromMinutes(5)).Select(
               (long x) =>
               {
                   var result = GetExchangeInfo2();
                   return result;
               }).Where(result => result.Success).Select(result => result.Data);
        }
        ...
        private const int GetAccountInfoWeight = 1;
        ...
        public async Task<Binance.AccountInfo> GetAccountInfoAsync()
        {
            ...
            var request = new RestSharp.RestRequest(GetAccountInfoEndpoint, RestSharp.Method.GET);
            var result = await RequestSignedApiAsync<Binance.AccountInfo>(request, GetAccountInfoWeight, offset);
            return result;
        }
        ...
        public async Task<Binance.DepositHistory> GetDepositHistoryAsync(string asset = null)
        {
            var serverTime = await GetServerTime();
            var offset = serverTime - DateTime.UtcNow;
            var request = new RestSharp.RestRequest(GetDepositHistoryEndpoint, RestSharp.Method.GET);
            if (asset != null)
            {
                request.AddQueryParameter("asset", asset);
            }
            var result = await RequestSignedApiAsync<Binance.DepositHistory>(request, GetDepositHistoryWeight, offset);
            return result;
        }
        ...
        public async Task<List<Binance.Order>> GetOpenOrdersAsync(string symbol = null)
        {
            var serverTime = await GetServerTime();
            var offset = serverTime - DateTime.UtcNow;
            var request = new RestSharp.RestRequest(GetOpenOrdersEndpoint, RestSharp.Method.GET);
            if (symbol != null)
            {
                request.AddParameter("symbol", symbol);
            }
            var result = await RequestSignedApiAsync<Binance.Order[]>(request, symbol != null ? 1 : GetOpenOrdersWeight, offset);
            return result.ToList();
        }

        public async Task<Binance.NewOrderResponseResult> PlaceOrderAsync(string symbol, TradeSide side, Binance.OrderType type, decimal amount, decimal? price = null, decimal? stopPrice = null, Binance.TimeInForce tif = Binance.TimeInForce.GTC, string newClientOrderId = null)
        {
            var serverTime = await GetServerTime();
            var offset = serverTime - DateTime.UtcNow;
            var request = new RestSharp.RestRequest(PlaceOrderEndpoint, RestSharp.Method.POST);
            request.AddQueryParameter("symbol", symbol);
            ...
            var result = await RequestSignedApiAsync<Binance.NewOrderResponseResult>(request, PlaceOrderWeight, offset);
            return result;
        }
        ...
        public IObservable<Binance.WsCandlestick> ObserveKlineCandlesticks(IEnumerable<string> symbols, string interval)
        {
            // A single connection to stream.binance.com is only valid for 24 hours; expect to be disconnected at the 24 hour mark
            const string url = "wss://stream.binance.com:9443";
            const string req = "/stream?streams=";
        ...
        {
            return OnTickerSocketMessage2(p.EventArgs.Message);
        }

        IList<Binance.WsPriceTicker24hr> OnTickerSocketMessage2(string message)
        {
            var tickers = JsonConvert.DeserializeObject<IList<Binance.WsPriceTicker24hr>>(message);
            return tickers;
        }
