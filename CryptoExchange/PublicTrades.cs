        ...
        void ViewModel_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            switch (e.PropertyName)
            {
                case nameof(viewModel.CurrentSymbolInformation):
                    ...
                    DisplayAssetInfo(viewModel.CurrentSymbolInformation);
                    break;
            }
        }

        internal async void DisplayAssetInfo(SymbolInformation si)
        {
            if (si.CmcId != -1)
            {
                label5.Text = si.CmcName;
                var client = new System.Net.Http.HttpClient();
                var result = await client.GetByteArrayAsync(si.ImageUrl);
                image1.Pixbuf = new Gdk.Pixbuf(result);
                //var client = new RestSharp.RestClient();
                //var req = new RestSharp.RestRequest(si.ImageUrl, RestSharp.Method.GET);
                //var resp = await client.ExecuteTaskAsync(req);
                //image1.Pixbuf = new Gdk.Pixbuf(resp.RawBytes);
            }
            else
            {
                label5.Text = si.BaseAsset;
                image1.Pixbuf = null;
            }
        }
